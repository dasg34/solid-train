---
name: tv-shopping-decision-support
description: "Plan, assess, and implement a Korea-first TV shopping decision support surface for the Samsung Tizen GenUI PoC. Use when the user asks for product comparison, top picks, review summaries, or one clear next buying action."
---

# TV Shopping Decision Support
...
```bash
python3 scripts/generate_shopping_a2ui.py
python3 scripts/generate_shopping_a2ui.py --state loading
python3 scripts/generate_shopping_a2ui.py --state error
```
...
1. Anchor the user intent around these requests: `상품 비교해줘`, `뭐 사는 게 좋아?`, `쇼핑 추천 보여줘`.
2. Choose the surface shape early. Default to `split comparison` and optimize for 10-foot readability.
3. Reuse `scripts/generate_shopping_a2ui.py` instead of hand-writing A2UI payloads for every turn.
4. Prioritize these payloads first: `product comparison`, `top picks`, `review summary`, `single next action`.
5. Start with `mock-first, catalog-later` and expand to live data only after the dependency list in `references/feasibility.md` is stable.
...
- Delivery phase: `P1`
- Recommended surface: `split comparison`
- Preferred data strategy: `mock-first, catalog-later`
