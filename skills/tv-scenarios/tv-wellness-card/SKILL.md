---
name: tv-wellness-card
description: "Plan, assess, and implement a Korea-first TV wellness card surface for the Samsung Tizen GenUI PoC. Use when the user asks for activity goals, sleep summaries, stretch prompts, or a lightweight wellness reminder."
---
...
```bash
python3 scripts/generate_wellness_a2ui.py
python3 scripts/generate_wellness_a2ui.py --state loading
python3 scripts/generate_wellness_a2ui.py --state error
```
...
1. Anchor the user intent around these requests: `건강 카드 보여줘`, `오늘 활동량 알려줘`, `스트레칭 안내해줘`.
2. Choose the surface shape early. Default to `center card` or `ambient status` and optimize for 10-foot readability.
3. Reuse `scripts/generate_wellness_a2ui.py` instead of hand-writing A2UI payloads for every turn.
4. Prioritize these payloads first: `activity goal`, `sleep summary`, `stretch prompt`, `guided session launch point`.
5. Start with `mock-first, health-sync-later` and expand to live data only after the dependency list in `references/feasibility.md` is stable.
...
- Delivery phase: `P2`
- Recommended surface: `center card` or `ambient status`
- Preferred data strategy: `mock-first, health-sync-later`
