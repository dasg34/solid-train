---
name: tv-smart-home-summary
description: "Plan, assess, and implement a Korea-first TV smart home summary surface for the Samsung Tizen GenUI PoC. Use when the user asks for doors, lights, indoor climate, air quality, cameras, or overall home status."
---

# TV Smart Home Summary
...
```bash
python3 scripts/generate_smart_home_a2ui.py
python3 scripts/generate_smart_home_a2ui.py --state loading
python3 scripts/generate_smart_home_a2ui.py --state error
```
...
1. Anchor the user intent around these requests: `집 상태 보여줘`, `스마트홈 요약해줘`, `문 잠겼는지 보여줘`.
2. Choose the surface shape early. Default to `modular dashboard` and optimize for 10-foot readability.
3. Reuse `scripts/generate_smart_home_a2ui.py` instead of hand-writing A2UI payloads for every turn.
4. Prioritize these payloads first: `doors`, `lights`, `temperature`, `air quality`, `cameras`, `unusual alerts`.
5. Start with `mock-first, home-platform-later` and expand to live data only after the dependency list in `references/feasibility.md` is stable.
...
- Preferred data strategy: `mock-first, home-platform-later`
