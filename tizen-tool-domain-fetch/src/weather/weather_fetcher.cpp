...
#ifdef TIZEN_TOOL_DOMAIN_FETCH_PROJECT_ROOT
  return TIZEN_TOOL_DOMAIN_FETCH_PROJECT_ROOT;
#else
  return ".";
#endif
}
...
std::vector<std::filesystem::path> CandidateFixturePaths(
  ...
  if (const char* env_root = std::getenv("TIZEN_TOOL_DOMAIN_FETCH_FIXTURE_ROOT");
...
#ifdef TIZEN_TOOL_DOMAIN_FETCH_DEFAULT_FIXTURE_ROOT
  {
    const std::filesystem::path root(TIZEN_TOOL_DOMAIN_FETCH_DEFAULT_FIXTURE_ROOT);
    candidates.push_back(root / file_name);
    candidates.push_back(root / "fixtures" / file_name);
  }
#endif
